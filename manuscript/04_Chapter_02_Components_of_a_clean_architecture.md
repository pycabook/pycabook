...
The word API is of uttermost importance in a clean architecture. Every layer may be accessed by elements living in inner layers by an API, that is a fixed[^footnote_fr-68478dee-1] collection of entry points (methods or objects).
...
[^footnote_fr-68478dee-1]: Here "fixed" means "the same among every implementation". An API may obviously change in time.
